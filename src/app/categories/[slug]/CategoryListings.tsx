...
export function CategoryListings({
  ...
  const [page, setPage] = useState(2); // next page to load (1 already loaded)
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="relative">
          ...
          <input
            ...
            placeholder={`Search ${categoryName} shops by name, city, or state...`}
        ...
        <div className="mt-6 rounded-2xl bg-white p-16 text-center shadow-[var(--card-shadow)] ring-1 ring-stone-900/[0.04] dark:bg-stone-900 dark:ring-stone-700">
          ...
          <p className="mt-4 text-stone-500 dark:text-stone-400">
            {search.trim()
              ...
              : `No ${categoryName.toLowerCase()} tattoo shops found.`}
          </p>
        </div>
      )}
      ...
      {hasMore && !search.trim() && (
        <div className="mt-8 flex justify-center">
          <button
            ...
            disabled={isPending}
            className="rounded-lg border border-stone-300 bg-white px-6 py-2.5 text-sm font-medium text-stone-700 shadow-sm transition-colors hover:bg-stone-50 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2 disabled:opacity-50 dark:border-stone-700 dark:bg-stone-800 dark:text-stone-300 dark:hover:bg-stone-700"
          >
