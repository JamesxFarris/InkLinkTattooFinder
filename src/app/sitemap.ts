...
export async function generateSitemaps() {
  try {
    ...
    const totalSitemaps = 1 + listingChunks + comboChunks;
    return Array.from({ length: totalSitemaps }, (_, i) => ({ id: i }));
  } catch {
    // DB unreachable at build time — use static fallback
...
export default async function sitemap(props: { id: Promise<string> }): Promise<MetadataRoute.Sitemap> {
  try {
    return await buildSitemap(props);
  } catch {
    // DB unreachable at build time (Railway internal hostname) — return empty; runtime will work
    return [];
  }
}

async function buildSitemap(props: { id: Promise<string> }): Promise<MetadataRoute.Sitemap> {
