...
export function ClaimButton({ listingId, existingClaimStatus, isOwned }: ClaimButtonProps) {
    ...
    return (
      <div className="rounded-xl border border-teal-500/30 bg-teal-500/10 p-6">
        <div className="flex items-center gap-2">
          <svg className="h-5 w-5 text-teal-400" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
            <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
          </svg>
          <span className="font-semibold text-teal-400">You own this listing</span>
        </div>
    ...
    return (
      <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-6">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-amber-400">Claim pending review</span>
        </div>
        <p className="mt-2 text-sm text-stone-400">
  ...
  if (existingClaimStatus === "denied") {
    return (
      <div className="rounded-xl border border-stone-700 bg-stone-900 p-6">
        <h3 className="font-semibold text-stone-100">Own this business?</h3>
        <p className="mt-2 text-sm text-stone-400">
  ...
  if (showForm) {
    return (
      <div className="rounded-xl border border-stone-700 bg-stone-900 p-6">
        <h3 className="font-semibold text-stone-100">Claim this Business</h3>
        <form onSubmit={handleSubmit} className="mt-4 space-y-3">
          {error && (
            <div className="rounded-lg bg-red-500/10 p-2 text-sm text-red-400">
              {error}
            </div>
          )}
          <div>
            <label htmlFor="claim-message" className="block text-sm text-stone-400">
            ...
            <textarea
              ...
              rows={3}
              className="mt-1 block w-full rounded-lg border border-stone-700 bg-stone-800 px-3 py-2 text-sm text-stone-100 placeholder-stone-500 focus:border-teal-500 focus:outline-none focus:ring-1 focus:ring-teal-500"
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="rounded-lg border border-stone-700 px-4 py-2 text-sm text-stone-400 hover:border-stone-600"
            >
              Cancel
            </button>
          </div>
